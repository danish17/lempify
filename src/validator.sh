...
function check_nginx() {
	...
	if [ -f /usr/sbin/nginx ]; then
		...
		echo "/usr/sbin/nginx file found." >> "$PROJECT_ROOT/logs/$filename" 2>&1
		return 1
	else
		warning "Nginx not detected."
		return 0
	fi
}
...
function check_mysql() {
	...
	if [ -f /etc/init.d/mysql ]; then
		...
		echo "No /usr/sbin/nginx file found." >> "$PROJECT_ROOT/logs/$filename" 2>&1
		return 1
	else
		warning "MySQL not detected."
		return 0
	fi
}
...
function check_php() {
	...
	if [ -f /usr/bin/php ]; then
		...
		echo "PHP $PHP_VERSION installation detected." >> "$PROJECT_ROOT/logs/$filename" 2>&1
		return 1
	else
		warning "PHP not detected."
		return 0
	fi
}